...
- 2026-08-04 重新核对 OOXML 语义和 WPS 网格视图对比后，网格对齐规则修正为：先计算字体、自动/多倍行距或最小值行距形成的自然行高，再向上量化为最少整数个 `linePitch`；固定值行距继续覆盖网格。不同倍数可能落在同一网格档位，跨过阈值后才多占一个完整网格。旧的 61 页数据不能作为本次修正后的结果。
- 字体包安装由部署方单独处理，不在后续代码排查范围；但比较时必须确认两端实际使用的字体、字号和字体槽一致，避免把字体回退误判为排版算法问题。
- 后续界面和用户可见文案统一使用“中文版式”“中文排版”或具体功能名称，不使用笼统的地域性功能标题。
- 不回退已经验证的中西文字体分槽、四个段落属性、文档网格属性链路和标点压缩实现；网格行距与段距的布局规则按最新 WPS 直接验证继续修正，不提前扩展新的设置界面。
...
实现状态：代码、浏览器 QUnit、core C2/C3 静态审查和官方 build-tools 编译完成；2026-08-04 已按自然行高到整数网格节距的规则修正自动/多倍行距，待部署、多节 DOCX 和更多 Microsoft Word/WPS 样本验收。
...
- 行网格保留自动/多倍行距形成的自然行高，并向上量化为最少整数个网格节距；显式磅值和行单位段距仍参与段落流布局，但对齐段落的最终基线不能停在基础网格之间；固定值行距覆盖网格。
