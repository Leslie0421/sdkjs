...
- 2026-08-03 的 WPS 直接交互验证证明，段落与网格对齐时 1 倍、1.5 倍、2 倍自动行距不应改变基线间距；sdkjs 已据此改为由网格节距接管自动行距，并取消磅值段距绕过基线对齐的特殊分支。该修正尚未部署，旧的 61 页数据不能作为修正后的结果。
...
实现状态：代码、浏览器 QUnit、core C2/C3 静态审查和官方 build-tools 编译完成；2026-08-03 已修正自动行距和段落基线的 WPS 兼容语义，待部署、多节 DOCX 和更多 Microsoft Word/WPS 样本验收。
...
- 行网格接管 1/1.5/2 倍等自动行距；显式磅值段前/段后间距不能让基线离开网格，行单位段距继续保留分数语义。
