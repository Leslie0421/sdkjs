...
- `word/Editor/Paragraph_Recalculate.js`
  - 段落和 Run 启用 `SnapToGrid` 且当前节存在行网格时，先保留字体度量、自动/多倍行距或最小值行距形成的自然行高，再向上量化为能够容纳该高度的最少整数个 `linePitch`。
  - 移除自动行距和 `beforeLines` / `afterLines` 对基础行网格的分数细分；段距属性仍参与段落流布局，但后续基线最终对齐到完整网格。
  - 固定值行距继续按 OOXML 规则覆盖文档行网格；段落/Run 关闭网格、表格 `adjustLineHeightInTable` / `doNotSnapToGridInCell` 等覆盖规则保持不变。
- `tests/word/document-calculation/paragraph/line-height.js`
  - 对 1、1.5、2、2.5、3、4 倍行距分别测量关闭网格时的自然基线距离，再验证开启网格后等于 `ceil(自然行高 / linePitch) × linePitch`。
  - 覆盖不同倍数落在同一网格档位、较大倍数进入后续档位、跨段落连续性、磅值段距、表格兼容开关，以及固定 28 磅行距在开关网格前后均不取整。

### 原因和验证

- WPS 网格视图对比显示，同一段落开启网格对齐后相邻文字行距离约为基础网格节距的整数倍，关闭后恢复非整数倍的自然行高；这说明网格约束的是垂直行布局，而不是直接清除段落的自动/多倍行距。
- 2026-08-03 提交 `95e423ed10` 把自动行距统一压成一个网格节距，只能解释部分样本，现已由本条目的自然行高量化规则纠正；旧部署的 61 页结果和该提交均不能代表本次修正后的分页。
- 段落计算 QUnit 26/26 用例、404/404 断言通过；相关 JavaScript `node --check`、`git diff --check` 通过，`build/node_modules/.bin/grunt compile-word` Closure 全量编译通过。
- 本次只修改 sdkjs 布局、测试和文档，不改变 core 协议或 web-apps 界面。

## 2026-08-03：让文档网格接管自动行距并保持段落基线对齐（已由 2026-08-04 条目纠正）
